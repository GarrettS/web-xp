...
This repo maintains two agent-file populations with different lifecycles:

- **_Contributor_ Agent Files** — at the **repo root**. Govern agents working on Web XP itself. Canonical content lives in `AGENTS.md` — Codex loads it natively; Claude Code loads it through a thin `CLAUDE.md` that imports it with `@AGENTS.md`. Hand-edited; commit and push directly.
...
The contributor side now runs on `AGENTS.md`; the user-side templates still emit `CLAUDE.md` / `CODEX.md` and migrate to `AGENTS.md` in a separate step. Changes to one population do not require changes to the other. Different audiences, different lifecycles.
...
├── AGENTS.md                   # Contributor Agent File (canonical) — for agents working on Web XP
├── CLAUDE.md                   # Contributor Agent File — thin `@AGENTS.md` import for Claude Code
